...
async function requireProspectWorkspaceAccess(
  req: Request,
...
async function sendAgentMail(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `AgentMail send failed (${response.status})${code ? ` [${code}]` : ''}`,
    )
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: event, error: eventError } = await supabase
      ...
      .select('id, prospect_id, opportunity_id, channel, event_type, content, metadata, external_id, workspace_id, occurred_at')
      .eq('id', outreachEventId)
      .single()
    if (eventError) throw eventError

    const { workspaceId, userId } = await requireProspectWorkspaceAccess(
      req,
      supabase,
      event.prospect_id,
    )
    ...
    if (event.channel !== 'email') {
      return json({ ok: false, reason: 'AgentMail dispatcher currently supports email only' }, 409)
    }
    if (event.event_type !== 'approved') {
      return json(
        { ok: false, reason: 'Explicit human approval is required before dispatch', current_event_type: event.event_type },
        409,
      )
    }

    const metadata = (event.metadata ?? {}) as Record<string, unknown>
    const dispatchStatus = metadata.dispatch_status
    if (event.external_id || dispatchStatus === 'sent') {
      return json({ ok: false, reason: 'Outreach event was already sent', outreach_event_id: event.id }, 409)
    }
    if (dispatchStatus === 'sending') {
      return json(
        { ok: false, reason: 'Dispatch is already in progress or requires recovery review', outreach_event_id: event.id },
        409,
      )
    }
    ...
    const recipient = typeof profileData.contact_email === 'string'
      ? profileData.contact_email.trim().toLowerCase()
      : ''
    if (!recipient || !recipient.includes('@')) {
      return json({ ok: false, reason: 'No verified prospect contact email is available' }, 409)
    }
    ...
    if (suppression) {
      return json(
        { ok: false, reason: 'Recipient is suppressed from outreach', suppression_reason: suppression.reason },
        409,
      )
    }
    ...
    const subject =
      typeof metadata.subject === 'string' && metadata.subject.trim()
        ...
        : `ZA Media — ${profile.canonical_name}`
    ...
    if (!claimed) {
      return json({ ok: false, reason: 'Outreach event could not be claimed for dispatch; it may have changed concurrently' }, 409)
    }

    let providerResult: { message_id?: string; thread_id?: string }
    try {
      providerResult = await sendAgentMail(
        agentMailApiKey,
        agentMailInboxId,
        recipient,
        subject,
      ...
      await supabase
        ...
        .update({
          metadata: {
            ...
            dispatch_error: sendError instanceof Error ? sendError.message : 'Unknown AgentMail error',
          },
        })
        .eq('id', event.id)
        .eq('workspace_id', workspaceId)
    ...
    const { data: sentEvent, error: sentError } = await supabase
      ...
      .eq('workspace_id', workspaceId)
      .is('external_id', null)
      .select('id,prospect_id,opportunity_id,channel,event_type,external_id,metadata,occurred_at')
      .single()
