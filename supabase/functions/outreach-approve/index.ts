...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!['drafted', 'reviewed'].includes(draft.event_type)) {
      return json({ ok: false, error: 'Only drafted/reviewed outreach can be approved', event_type: draft.event_type }, 409)
    }
    ...
    if (draft.channel !== 'email') {
      return json({ ok: false, error: 'Only email approval is wired to the current outbound dispatcher', channel: draft.channel }, 409)
    }
    ...
    if (!['identified', 'reviewed', 'approved'].includes(opportunity.status)) {
      return json({ ok: false, error: 'Opportunity is not eligible for approval', opportunity_status: opportunity.status }, 409)
    }
    ...
    if (existingApproved?.length) {
      return json({ ok: true, already_approved: true, approved_event: existingApproved[0] })
    }
    ...
    const { data: approvedEvent, error: approvedError } = await supabase
      .from('prospect_outreach_events')
      .insert({
      ...
      .select('id,prospect_id,opportunity_id,workspace_id,channel,event_type,content,metadata,occurred_at')
      .single()
    ...
    return json({
      ...
      next_action: 'Call outreach-send with the approved_event.id after human review is complete.',
    })
  } catch (error) {
