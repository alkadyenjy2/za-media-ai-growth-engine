...
function Dashboard({ initialPage = 'overview', onNavigate }: { initialPage?: PageKey; onNavigate: (page: PageKey) => void }) {
  ...
  const content = page === 'overview' ? <OverviewPage /> : page === 'leads' ? <LeadsPage /> : page === 'outreach' ? <OutreachPage /> : page === 'conversations' ? <ConversationsPage /> : page === 'campaigns' ? <CampaignsPage /> : page === 'companies' ? <CompaniesPage /> : page === 'income' ? <IncomePage /> : <AuditPage />
...
const dashboardPages: Record<string, PageKey> = {
  ...
  '/app/campaigns': 'campaigns', '/app/outreach': 'outreach', '/app/companies': 'companies', '/app/income': 'income', '/app/activity': 'audit',
}
