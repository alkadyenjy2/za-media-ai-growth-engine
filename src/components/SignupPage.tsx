...
export const SignupPage: React.FC<SignupPageProps> = ({ onSwitchToLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error: authError } = await signUpWithEmail(email.trim(), password, fullName.trim());
      if (authError) {
        setError(authError.message || 'Failed to create account.');
      } else {
        if (isSupabaseConfigured) {
          setSuccess('Account created successfully! Check your email to confirm registration or sign in directly.');
        } else {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center p-4 relative overflow-hidden font-sans text-slate-100">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-gradient-to-tr from-purple-600/15 via-indigo-600/20 to-blue-600/15 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="w-full max-w-md relative z-10 space-y-6">
        ...
        <div className="text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Join the AI Revenue & Growth Operations Platform
            </p>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800/90 rounded-2xl p-6 shadow-2xl shadow-black/80 space-y-5">
          ...
          {success && (
            <div className="p-3.5 bg-emerald-950/70 border border-emerald-800/80 rounded-xl text-emerald-300 text-xs flex items-start gap-2.5">
