...
export function Sidebar({ active, onNavigate, open, onClose }: { active: PageKey; onNavigate: (page: PageKey) => void; open: boolean; onClose: () => void }) {
  return <aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col bg-neutral-950 text-white transition-transform duration-200 lg:static lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
    <div className="flex h-20 items-center justify-between border-b border-white/10 px-6"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary-500 font-display text-lg font-bold">Z</div><div><div className="font-display font-bold tracking-wide">ZE OPS</div><div className="text-[10px] font-medium uppercase tracking-[0.18em] text-neutral-400">AI Operations</div></div></div><button onClick={onClose} className="lg:hidden text-neutral-400"><X size={20}/></button></div>
    ...
    <div className="border-t border-white/10 p-4"><div className="rounded-xl bg-white/5 p-3"><div className="flex items-center gap-2"><span className="h-2 w-2 animate-pulse rounded-full bg-accent-400"/><span className="text-xs font-medium text-neutral-300">Systems operational</span></div><p className="mt-2 text-[11px] leading-4 text-neutral-500">AI workflows are monitoring your pipeline.</p></div></div>
  </aside>
}
